...
export const load: PageServerLoad = async ({ url }) => {
    ...
    const barResultsRaw = await db
        ...
        .from(transaction)
        .innerJoin(building, eq(transaction.buildingId, building.id))
        .innerJoin(transactionType, eq(transaction.transactionTypeId, transactionType.id))
        .where(
            and(
                gte(transaction.date, startDate),
    ...
    barResultsRaw.forEach((row) => {
        if (!barMap.has(row.label)) {
            barMap.set(row.label, { label: row.label, value: 0, negativeValue: 0 });
        }
        const entry = barMap.get(row.label)!;
        if (row.isExpense) {
